...
def test_is_render_up_to_date():
    if not output_template_path.is_file():
        raise IOError(f'{str(output_template_path)} does not exist. Run build.py to populate it and include it in your commit.')
    ...
    for line_tuple in itertools.zip_longest(file_lines, desired_lines):
        if line_tuple[0] != line_tuple[1]:
            raise IOError(f'{str(output_template_path)} is not up-to-date. Run build.py to update it and include it in your commit..')
